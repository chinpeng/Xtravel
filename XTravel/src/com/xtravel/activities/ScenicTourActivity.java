 ...
 * 
 * @author 王中强
 * @version 1.0.0 (10/09 导入)
 */
public class ScenicTourActivity extends PLView {
	/** member variables */
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		this.setListener(new PLViewListener() {
			...
			public void onDidClickHotspot(PLIView view, PLIHotspot hotspot, CGPoint screenPoint, PLPosition scene3DPoint) {
				Toast.makeText(view.getActivity().getApplicationContext(), String.format("select the hotspot with ID %d", hotspot.getIdentifier()), Toast.LENGTH_SHORT).show();
				if (9 == hotspot.getIdentifier()) {
					createAlertDiaglog9();
				}
			}
	...
	private void createAlertDiaglog9() {
		...
		builder.setPositiveButton("登陆", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				EditText et_username = (EditText) view.findViewById(R.id.editText1);
				EditText et_password = (EditText) view.findViewById(R.id.editText2);
				String username = et_username.getText().toString();
				String password = et_password.getText().toString();
				Toast.makeText(ScenicTourActivity.this, username + "--" + password, Toast.LENGTH_SHORT).show();
			}
		});
	...
	protected View onContentViewCreated(View contentView) {
		...
		// Load layout
		ViewGroup mainView = (ViewGroup) this.getLayoutInflater().inflate(R.layout.activity_scenic_tour, null);
		// Add 360 view
